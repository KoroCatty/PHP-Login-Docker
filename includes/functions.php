  ...
  if (isset($_COOKIE['_ucv_'])) {
    ...
    if (mysqli_num_rows($query_con) == 1) {
      ...
      if ($expire_date >= $current_date) {
        ...
        // 下の関数にトークンを渡して、ユーザー名を取得し、セッションに保存
        $name = selectUserByToken($selector);
        echo $_SESSION['name'] = $name; // echo しないと表示されない
        return true;
      }
    }
  }
...
// Get user first & last name by token
function selectUserByToken($token)
{
  ...
  $queryAll = "SELECT * FROM users WHERE user_name = '$user_name'";
  $query_conAll = mysqli_query($connection, $queryAll);
  if (!$query_conAll) {
    die("Query Failed" . mysqli_error($connection));
  }

  $resultAll = mysqli_fetch_assoc($query_conAll);
  return $resultAll['first_name'] . " " . $resultAll['last_name'];
}
